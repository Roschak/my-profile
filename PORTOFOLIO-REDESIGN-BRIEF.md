...
> Isi bagian ini setelah audit awal — sesuaikan dengan framework yang dipakai portofolio saat ini (React/Next.js/Vue/HTML-CSS-JS biasa, dsb).

- Framework saat ini: `___________`
- Library animasi yang akan dipakai: `GSAP + ScrollTrigger` / `Framer Motion` / `Lenis` (pilih sesuai kompatibilitas stack)
- Library particle: `tsparticles` / custom canvas (pilih yang paling ringan)
- Font: Serif klasik untuk judul (contoh arah: Cinzel, Playfair Display, Cormorant) + Sans-serif modern untuk body (contoh arah: Inter, Manrope) — final pilihan menyesuaikan brand
- Asset yang perlu dibuat baru: custom icon set, illustration/background per section, texture marmer, particle sprite
